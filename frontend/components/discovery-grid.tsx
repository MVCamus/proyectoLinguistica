...
import { Search, Hash, X, RefreshCw, Link, Loader2, Download, Mic, Clock, AlertCircle, CheckCircle2, ListOrdered, Trash2 } from 'lucide-react'
...
export function DiscoveryGrid({ videos, processingQueue, onApprove, onReject, onCancelQueue, onReintentar, onSubmitUrls, loading }: DiscoveryGridProps) {
  ...
  const errores = processingQueue.filter((i) => i.status === 'error')
  ...
  const statusRow = (item: ProcessingItem) => {
    ...
    return (
      <div key={item.id} className="flex flex-col gap-1.5 p-3 rounded-md bg-secondary/30 border border-border/50 text-sm">
        <div className="flex items-center gap-3">
          <div className={item.status === 'error' ? info.color : info.color + ' animate-pulse'}>{info.icon}</div>
          ...
          {item.status === 'error' ? (
            <button
              ...
              className="h-6 w-6 p-0 text-muted-foreground hover:text-green-500 flex items-center justify-center rounded hover:bg-green-500/10 transition-colors"
              title="Reintentar"
            >
        ...
        {item.status === 'error' && item.errorMessage && (
          <div className="ml-7 text-xs text-red-400 bg-red-950/20 border border-red-500/20 p-2 rounded">
            {item.errorMessage}
          </div>
        )}
      </div>
      ...
      {processingQueue.length > 0 && (
        <div className="px-6 pb-6">
          ...
          <div className="space-y-2">
            ...
            {errores.map((item) => (
              <div key={item.id}>
                {errores.indexOf(item) === 0 && <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1"><AlertCircle className="h-3 w-3 text-red-500" /> Con error ({errores.length})</p>}
                {statusRow(item)}
              </div>
            ))}
          </div>
